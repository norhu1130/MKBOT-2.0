...
async def on_ready():
     print("로그인을완료함" % ())
     await client.change_presence(game=discord.Game(name="MK 도움말", type=0))
...
async def on_message(message):
                ...
                if a == "":
                   ...
                   await client.send_message(message.channel, embed=embed)
                if message.content.startswith('MK 개발프로그램'):
                   await client.send_message(message.channel, "PYthon  비주얼코드")  
                if message.content.startswith('MK 개발됄기능'):
                  await client.send_message(message.channel, "개발됄기능 데이터베이스활용 문의기능")    
